...
export default function HomePage() {
  ...
  return (
    <div className="flex flex-col">
      ...
      {/* ── Hero ─────────────────────────────────────────── */}
      <section className="relative overflow-hidden mesh-gradient py-24 md:py-36">
        {/* Decorative blobs */}
        ...
        <div className="container relative mx-auto px-4 text-center">
          ...
          <div className="inline-flex items-center gap-2 rounded-full border border-primary/20 bg-primary/5 px-5 py-2 text-sm font-medium text-primary mb-8 shadow-sm">
            ...
            <span>ISO 9001 &amp; ISO 27001 Certified · {locale === "sq" ? "Nga 2012" : "Since 2012"}</span>
          </div>
          ...
          <p className="text-lg md:text-xl text-muted-foreground mb-10 max-w-3xl mx-auto leading-relaxed">
            {locale === "sq"
              ? "Zgjidhje IT gjithëpërfshirëse për bizneset shqiptare — nga mbështetja ditore deri te transformimi dixhital. 500+ klientë na besojnë."
              : "Comprehensive IT solutions for Albanian businesses — from daily support to digital transformation. 500+ clients trust us."}
          </p>
          ...
          <div className="flex flex-col sm:flex-row flex-wrap gap-4 justify-center mb-12">
            ...
            <Button variant="outline" asChild>
              <Link href={`${lp}/sherbime`}>
                {locale === "sq" ? "Shiko Shërbimet" : "View Services"}
              </Link>
            </Button>
          </div>
          ...
          <div className="flex flex-wrap items-center justify-center gap-6 text-sm text-muted-foreground">
            {[
              ...
              { icon: Trophy, val: "12+", label: locale === "sq" ? "Vite" : "Years" },
              { icon: HeadphonesIcon, val: "24/7", label: locale === "sq" ? "Mbështetje" : "Support" },
              { icon: TrendingUp, val: "98%", label: locale === "sq" ? "Kënaqësi" : "Satisfaction" },
            ].map((s) => (
              <div key={s.val} className="flex items-center gap-2">
                <s.icon className="h-4 w-4 text-primary" />
                <strong className="text-foreground font-bold">{s.val}</strong>
                <span>{s.label}</span>
              </div>
            ))}
          </div>
      ...
      <section className="border-y border-border/50 bg-gradient-to-b from-slate-50/90 to-white">
        <div className="container mx-auto max-w-6xl px-4 py-10 md:py-14">
          <div className="grid grid-cols-2 gap-3 sm:gap-4 lg:grid-cols-4 lg:gap-5">
            {[
              { value: "500+", label: locale === "sq" ? "Klientë Aktivë" : "Active Clients", icon: Users, color: "text-blue-600", bg: "bg-blue-50" },
              { value: "12+",  label: locale === "sq" ? "Vite Eksperiencë" : "Years Experience", icon: Clock, color: "text-amber-600", bg: "bg-amber-50" },
              { value: "50+",  label: locale === "sq" ? "Inxhinierë Certifikuar" : "Certified Engineers", icon: Trophy, color: "text-emerald-600", bg: "bg-emerald-50" },
              { value: "24/7", label: locale === "sq" ? "Mbështetje Teknike" : "Tech Support", icon: Zap, color: "text-violet-600", bg: "bg-violet-50" },
            ].map((stat) => (
              <div
                key={stat.value}
              ...
              >
                <div
                  className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-xl sm:h-12 sm:w-12 ${stat.bg}`}
                  aria-hidden
                >
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="mx-auto grid max-w-6xl grid-cols-1 gap-4 sm:grid-cols-2 sm:gap-4 lg:grid-cols-4 lg:gap-5">
            {serviceKeys.map((key) => {
              ...
              const col = serviceColors[key];
              const accent = serviceAccent[key];
              return (
                <Link key={key} href={`${lp}${serviceHrefs[key]}`} className="group flex h-full min-h-0">
            ...
            {testimonials.map((item) => (
              ...
              >
                ...
                <div className="flex items-center gap-3">
                  <div
                    className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-full ${item.color} text-white font-bold text-sm tracking-tight`}
                    aria-hidden
                  >
                    {item.initials}
                  </div>
                  <div className="min-w-0">
                    <p className="font-bold text-sm">{item.initials}</p>
